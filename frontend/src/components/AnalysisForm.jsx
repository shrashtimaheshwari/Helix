...
export default function AnalysisForm({ onResults, onLoadingChange, results = [] }) {
  ...
  return (
    <section className="bg-white rounded-2xl border border-slate-200 shadow-md p-8">
      ...
      <div className="mt-8 flex items-center gap-4">
        ...
        <button
          onClick={() => results.length > 0 && navigate("/risk", { state: { results } })}
          disabled={results.length === 0}
          className={`px-6 py-3 rounded-xl font-semibold transition-colors ${
            results.length > 0
              ? "bg-teal-600 text-white hover:bg-teal-500 cursor-pointer"
